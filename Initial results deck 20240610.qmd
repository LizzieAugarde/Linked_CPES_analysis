...
The England National Cancer Patient Experience Survey (NCPES) allows cancer patients to give feedback on the care they have received. Results from the survey can be used to improve the experience of cancer patients at a national, regional and local level.

NCPES is delivered to a sample of patients who receive inpatient care for their cancer during April-June of each year. NDRS links NCPES response data to the cancer registry data. This allows for the analysis of clinical information for those completing the survey.

Linked CPES-registry data is currently available for NCPES years 2017-2022. Each year, approximately 50,000 responses to NCPES are received, meaning there is a total of 254,504 patients in the cohort for this analysis.

The survey asks the following questions:

Q: Which of the following options best describes how you think of yourself? Answers: Heterosexual or Straight/ Gay or Lesbian / Bisexual / Other / Prefer not to say / Don't know/not sure

Q: Is English your first language? Answers: Yes/No

Therefore, it is possible to use linked NCPES-registry data to analyse variation in clinical characteristics by sexuality and language status. These characteristics are not routinely recorded anywhere else in the cancer registry data, meaning this dataset offers a unique opportunity to identify any inequalities in outcomes by these characteristics. STUFF FROM PROPOSAL.
...
#####at some point add the %s in the text as dynamic text
```

Each year, approximately 1.4% of respondents describe their sexuality as 'gay or lesbian', 'bisexual' or 'other'. This equates to 3,660 respondents between 2017 and 2022.

Approximately 7.3% of respondents respond 'prefer not to say', 'don't know/not sure', or do not provide a response.

```{r sexuality_composition}
...
Each year, approximately 4.6% of respondents state that English is not their first language. This equates to 11,759 respondents between 2017 and 2022.

Approximately 4.1% of respondents do not provide a response to this question.

```{r language_composition}
...
## Sexuality and routes to diagnosis
...
A higher proportion of respondents who describe their sexuality as gay, lesbian, or other, are diagnosed via screening compared to the proportion of those describing themselves as heterosexual. Otherwise, the distribution of respondents across the different routes to diagnosis is very similar for heterosexual and sexual minority respondents.
...
## Language status and routes to diagnosis

A higher proportion of respondents who are non-native English speakers are diagnosed via emergency presentation, compared to native English speakers. A lower proportion are diagnosed via screening or USC referrals. Otherwise, the distribution of respondents across the different routes to diagnosis is similar for native English speakers and non-native English speakers.
...
Analysis of CPES responses linked to stage at diagnosis information from the NDRS cancer registry indicates that a lower proportion of respondents describing their sexuality as gay, lesbian, or other, are diagnosed at a later stage (stage 3 or 4), compared to heterosexual respondents.
...
Analysis of CPES responses linked to stage at diagnosis information from the NDRS cancer registry indicates that respondents describing themselves as non-native English speakers were less likely to be diagnosed at a later stage than native English speaking respondents.
...
#graph 
ggplot(stage_language, aes(x = lang_stat, y = percent, fill = STAGE_2LEVEL)) +
  geom_bar(position = "dodge", stat = "identity") +   
  geom_text(position = position_dodge(width = 0.8), 
            aes(y = percent + 0.25, fill = STAGE_2LEVEL, label = percent_label,                  hjust = -0.15), angle = 90, size = 3) +   
  ggtitle("Percentage of respondents by stage at diagnosis and language status, 2017-2022") +   
  labs(x = "Language status",         
       y = "Percentage of respondents",         
